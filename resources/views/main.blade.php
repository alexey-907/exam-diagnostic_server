...
<body class="bg-[#FFFDF7] font-sans text-slate-800">
...
<div id="landing-page" class="bg-[#F8F5EE] min-h-screen flex flex-col justify-between">
    ...
    <main class="bg-[#F8F5EE] max-w-4xl mx-auto px-6 py-12 flex-grow flex flex-col justify-center items-center text-center">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-3xl mb-12 text-left">
            <div class="bg-white p-5 rounded-xl shadow-sm border border-bg-brand-btnSubmit">
        ...
        <form id="start-profile-form" class="space-y-5">
            ...
            <div id="step-1" class="space-y-5">
                <div>
                    ...
                    <select id="role" class="w-full bg-[#FFFDF7] border border-bg-brand-btnSubmit rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500/20 focus:border-[#ac6725] transition" required>
                ...
                <div>
                    ...
                    <select id="grade" class="w-full bg-[#FFFDF7] border border-bg-brand-btnSubmit rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500/20 focus:border-[#ac6725] transition" required>
                ...
                <div>
                    ...
                    <select id="examType" class="w-full bg-[#FFFDF7] border border-bg-brand-btnSubmit rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500/20 focus:border-[#ac6725] transition" required>
                ...
                <div class="relative">
                    ...
                    <input type="text" id="region" autocomplete="off" class="w-full bg-[#FFFDF7] border border-bg-brand-btnSubmit rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500/20 focus:border-[#ac6725] transition" placeholder="Начните вводить регион..." required>

                    <!-- Наш выпадающий блок подсказок -->
                    <div id="region-suggestions" class="absolute left-0 right-0 mt-1 bg-white border border-bg-brand-btnSubmit rounded-lg shadow-lg hidden z-50 max-h-48 overflow-y-auto"></div>
                </div>
            ...
            <div id="step-2" class="space-y-5 hidden">
                ...
                <div class="grid grid-cols-1 gap-3">
                    <!-- Вариант: ВУЗ -->
                    <label class="flex items-start p-3 bg-[#FFFDF7] border border-bg-brand-btnSubmit rounded-xl cursor-pointer hover:bg-[#dca96f] transition">
                ...
                <div class="flex gap-3 mt-4">
                    ...
                    <button type="button" data-next="3" class="w-2/3 bg-blue-600 hover:bg-blue-700 text-[#C98D52]font-semibold py-3 px-4 rounded-xl shadow-md transition mt-2">
                        Далее
                    </button>
            ...
            <div id="step-3" class="space-y-5 hidden">
                ...
                <div id="subjects-container" class="space-y-4">
                    @isset($subjects)
                        @foreach($subjects as $subject)
                            <div class="subject-row bg-[#FFFDF7] p-3 rounded-xl border border-bg-brand-btnSubmit flex flex-col sm:flex-row sm:items-center justify-between gap-3" data-code="{{ $subject->code }}">
                                ...
                                <div class="flex gap-2">
                                    <div class="w-1/2 sm:w-28">
                                        ...
                                        <select class="subject-grade w-full bg-white border border-bg-brand-btnSubmit rounded-lg p-1.5 text-sm" data-subject="{{ $subject->code }}" data-type="quarter" required>
                                    ...
                                    <div class="w-1/2 sm:w-28">
                                        ...
                                        <select class="subject-grade w-full bg-white border border-bg-brand-btnSubmit rounded-lg p-1.5 text-sm" data-subject="{{ $subject->code }}" data-type="year" required>
                                            <option value="5">5</option>
                                            <option value="4">4</option>
                                            <option value="3">3</option>
                                            <option value="2">2</option>
                ...
                <div class="flex gap-3 mt-4">
                    ...
                    <button type="button" data-next="4" class="w-2/3 bg-blue-600 hover:bg-blue-700 text-[#C98D52]font-semibold py-3 px-4 rounded-xl shadow-md transition mt-2">
                        Далее
                    </button>
                </div>
            </div>
            ...
            <div id="step-4" class="space-y-5 hidden">
                ...
                <div id="attitude-subjects-container" class="space-y-3.5 max-h-[48vh] overflow-y-auto pr-1">
                    @isset($subjects)
                        @foreach($subjects as $subject)
                            <div class="attitude-row bg-[#FFFDF7] p-4 rounded-xl border border-bg-brand-btnSubmit/60 space-y-3" data-code="{{ $subject->code }}">
                                <div class="flex items-center justify-between">
                                    <span class="font-semibold text-slate-800 text-sm flex items-center gap-2">{{ $subject->title }}</span>
                                </div>
                                <div class="unfold-group border-t border-bg-brand-btnSubmit/60 pt-2">
                                    ...
                                    <div class="unfold-content hidden flex-col gap-3 mt-3 pl-2 border-l-2 border-bg-brand-btnSubmit">
                                        @isset($questions)
                                            @foreach($questions as $question)
                                                @if ($question->step === 'att')
                                                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-3 rounded-lg border border-bg-brand-btnSubmit shadow-sm">
                                                        <label class="text-xs text-slate-600 font-medium max-w-md">{{ $question->question }}</label>
                                                        <div class="flex items-center gap-1 bg-[#FFFDF7] border border-bg-brand-btnSubmit/80 p-1 rounded-xl self-start sm:self-auto">
                                                            ...
                                                            @foreach(range(1, 5) as $val)
                                                                <label class="cursor-pointer">
                                                                    ...
                                                                    <div class="w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold text-slate-500 bg-white border border-bg-brand-btnSubmit peer-checked:border-blue-500 peer-checked:bg-blue-600 peer-checked:text-[#C98D52]hover:border-slate-300 transition">{{ $val }}</div>
                                                                </label>
                                                            @endforeach
            ...
            <div id="step-5" class="space-y-5 hidden">
                ...
                <div class="bg-brand-bg text-[#C98D52]p-8 rounded-2xl shadow-inner flex flex-col items-center justify-center min-h-[250px] relative overflow-hidden" id="wm-game-container">
                    ...
                    <div id="wm-input-zone" class="hidden text-center space-y-4 w-full max-w-sm">
                        ...
                        <input type="text" id="wm-user-answer" autocomplete="off" class="w-full bg-brand-bg border border-bg-brand-btnSubmit text-[#B0683663] border border-[#C98D52] rounded-xl p-3 text-center text-xl font-bold tracking-widest text-[#C98D52] focus:outline-none focus:border-[#ac6725] transition" placeholder="Введите ваш ответ через дефис">

                        <button type="button" id="submit-wm-answer-btn" class="w-full bg-brand-btnSubmit hover:bg-[#dca96f] text-[#C98D52]font-semibold py-2 px-4 rounded-xl transition">
                            Подтвердить ответ
                        </button>
                    </div>
            ...
            <div id="step-6" class="space-y-5 hidden">
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1">Вербальная память (VM)</label>
                ...
                <div class="bg-brand-bg text-[#C98D52]p-6 rounded-2xl shadow-inner min-h-[300px] flex flex-col items-center justify-center relative overflow-hidden" id="vm-verbal-container">
                    ...
                    <div id="vmv-show" class="hidden text-center space-y-4 w-full">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-5 gap-3 max-w-xl mx-auto py-2">
                            @foreach(['атом', 'рынок', 'закон', 'клетка', 'облако', 'договор', 'энергия', 'образ', 'эпоха', 'алгоритм'] as $word)
                                <span class="bg-brand-select border border-slate-300 text-[#C98D52] border border-[#C98D52] px-3 py-2 rounded-xl text-sm font-medium tracking-wide shadow-sm select-none">{{ $word }}</span>
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div id="vmv-immediate" class="hidden text-center space-y-4 w-full max-w-md">
                        ...
                        <textarea id="vmv-immediate-input" rows="3" class="w-full bg-brand-bg border border-bg-brand-btnSubmit text-[#B0683663] border border-[#C98D52] rounded-xl p-3 text-sm text-[#C98D52]focus:outline-none focus:border-[#ac6725] transition placeholder-slate-500" placeholder="Пример: атом, рынок, закон..."></textarea>
                        <button type="button" id="submit-vmv-immediate" class="w-full bg-brand-btnSubmit hover:bg-[#dca96f] text-[#C98D52]font-semibold py-2 px-4 rounded-xl transition">
                    ...
                    <div id="vmv-recognition" class="hidden text-center space-y-4 w-full max-w-xl">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 text-left py-2">
                            @foreach(['атом', 'камень', 'рынок', 'формула', 'клетка', 'энергия', 'письмо'] as $recWord)
                                <label class="flex items-center p-2.5 bg-brand-select border border-slate-300 text-[#B0683663] border border-[#C98D52] rounded-xl cursor-pointer hover:bg-slate-750 transition">
                                    <input type="checkbox" name="vmv_rec" value="{{ $recWord }}" class="rounded text-blue-600 mr-2.5 focus:ring-blue-500 bg-[#FEF5ED] border-slate-600">
                                    <span class="text-xs font-medium text-[#B06836] ">{{ $recWord }}</span>
                                </label>
                            @endforeach
                        </div>
                        <button type="button" id="submit-vmv-recognition" class="w-full bg-brand-btnSubmit hover:bg-[#dca96f] text-[#C98D52]font-semibold py-2 px-4 rounded-xl transition mt-2">
                    ...
                    <div id="vmv-categorization" class="hidden text-center space-y-4 w-full max-w-xl">
                        ...
                        <div class="space-y-2.5 text-left max-h-[180px] overflow-y-auto pr-1">
                            @foreach(['атом', 'рынок', 'закон', 'клетка', 'облако', 'договор', 'энергия', 'образ', 'эпоха', 'алгоритм'] as $catWord)
                                <div class="flex items-center justify-between bg-brand-select border border-slate-300 text-[#C98D52] border border-[#C98D52] p-2 rounded-xl">
                                    <span class="text-xs font-medium text-[#C98D52] -400 pl-2">{{ $catWord }}</span>
                                    <select name="cat_{{ $catWord }}" class="bg-[#FEF5ED] border border-slate-600 text-xs rounded-lg p-1.5 text-[#C98D52]w-36">
                        ...
                        <button type="button" id="submit-vmv-categorization" class="w-full bg-brand-btnSubmit hover:bg-[#dca96f] text-[#C98D52]font-semibold py-3 px-4 rounded-xl transition shadow-md">
            ...
            <div id="step-7" class="space-y-5 hidden">
                ...
                <div class="bg-brand-bg text-[#C98D52]p-6 shadow-inner min-h-[220px] flex flex-col justify-between relative overflow-hidden" id="lr-game-container">

                    <!-- Контейнер текущего вопроса -->
                    <div class="space-y-4">
                        <!-- Код и текст вопроса -->
                        <p id="lr-question-text" class="px-3 text-sm font-medium text-[#B06836] leading-relaxed"></p>
            ...
            <div id="step-8" class="space-y-5 hidden">
                ...
                <div class="bg-brand-bg text-[#C98D52]p-6 rounded-2xl shadow-inner min-h-[220px] flex flex-col justify-between relative overflow-hidden" id="ar-game-container">
                    ...
                    <div class="w-full max-w-xs mx-auto space-y-3">
                        <input type="text" id="ar-user-answer" autocomplete="off" class="w-full bg-brand-bg border border-bg-brand-btnSubmit text-[#B0683663] border border-[#C98D52] rounded-xl p-3 text-center text-xl font-bold tracking-widest text-[#C98D52] focus:outline-none focus:border-indigo-500 transition" placeholder="Введите ваш ответ">

                        <button type="button" id="ar-submit-answer-btn" class="w-full bg-brand-btnSubmit hover:bg-[#B06836] text-[#C98D52]font-semibold py-2 px-4 rounded-xl transition">
                            Подтвердить ответ
                        </button>
                    </div>
            ...
            <div id="step-9" class="space-y-5 hidden">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="bg-[#FFFDF7] border border-bg-brand-btnSubmit rounded-2xl p-4 flex flex-col justify-center">
                        <span class="text-[10px] uppercase font-bold tracking-wider text-teal-600 mb-2">Прочитай текст:</span>
                        <p class="text-xs px-3 md:text-sm text-slate-700 leading-relaxed font-medium bg-white p-3.5 rounded-xl border border-bg-brand-btnSubmit shadow-sm select-none">
                            «Выбор предметов для экзамена зависит не только от того, какой предмет нравится. Важно учитывать цель после школы, текущий уровень знаний, время на подготовку и требования образовательной программы. Если предмет интересен, но база слабая, его не нужно сразу исключать: сначала стоит пройти диагонстику и оценить, сколько времени потребуется на подготовку.»
                        </p>
                    </div>

                    <!-- Правая колонка: Игровой терминал с вопросами -->
                    <div class="bg-brand-bg text-[#C98D52]p-5  shadow-inner min-h-[250px] flex flex-col justify-between relative overflow-hidden" id="vr-game-container">

                        <div id="vr-questions-wrapper" class="space-y-3.5 w-full flex-grow flex flex-col justify-between">
                            <div>
                                <!-- Текст вопроса -->
                                <p id="vr-question-text" class="px-3 text-xs md:text-sm font-medium text-[#B06836] leading-relaxed mb-3"></p>
                                <!-- Варианты ответов -->
            ...
            <div id="step-10" class="space-y-5 hidden">
                ...
                <div class="bg-brand-bg text-[#C98D52]p-6 rounded-2xl shadow-inner min-h-[220px] flex flex-col justify-between relative overflow-hidden" id="sp-game-container">
                    ...
                    <div id="sp-questions-wrapper" class="space-y-4 w-full flex-grow flex flex-col justify-between">
                        <div class="space-y-3">
                            <p class="text-xs uppercase tracking-wider text-[#C98D52] font-bold px-3 py-1.5 rounded-full inline-block">Мысленно представь схему или объект</p>
                            <!-- Текст вопроса -->
                            <p id="sp-question-text" class="px-3 text-sm font-medium text-[#B06836] leading-relaxed"></p>
                        </div>

                        <!-- Зона ввода ответа -->
                        <div class="w-full max-w-xs mx-auto space-y-3 pt-2">
                            <input type="text" id="sp-user-answer" autocomplete="off" class="w-full bg-brand-bg border border-bg-brand-btnSubmit text-[#B0683663] border border-[#C98D52] rounded-xl p-3 text-center text-sm font-bold tracking-wide text-[#C98D52] focus:outline-none focus:border-purple-500 transition" placeholder="Введите ваш ответ">
            ...
            <div id="step-11" class="space-y-5 hidden">
                ...
                <div class="bg-brand-bg text-[#C98D52]p-6 shadow-inner min-h-[260px] flex flex-col justify-between relative overflow-hidden" id="att-game-container">
                    ...
                    <div id="att-questions-wrapper" class="space-y-4 w-full flex-grow flex flex-col justify-between">
                        <div class="space-y-2">
                            <p id="att-badge-text" class="text-xs uppercase tracking-wider text-[#B06836]-400 font-bold px-3 py-1.5 rounded-full inline-block">Тест на концентрацию</p>
                            <!-- Текст вопроса или задания -->
                            <p id="att-question-text" class="px-3 text-sm font-medium text-[#B06836] leading-relaxed"></p>
                        </div>
            ...
            <div id="step-12" class="space-y-5 hidden">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3" id="clusters-container">
                    ...
                    <label class="cluster-card flex items-start p-3 bg-white border border-bg-brand-btnSubmit rounded-xl cursor-pointer hover:border-blue-400 hover:bg-blue-50/20 transition relative">
            ...
            <div id="step-13" class="space-y-6 hidden">
                ...
                <div class="space-y-3 border-t border-bg-brand-btnSubmit/60 pt-4">
                ...
                <div class="flex gap-3 mt-6 pt-2 border-t border-bg-brand-btnSubmit">
